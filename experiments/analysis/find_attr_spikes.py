...
def top_indices(saved_file,groups,metrics,mode="attn",head_idx=0):
...
def indices_to_substrings(index_list,motif_fasta,df):
    
    storage = []
    sequences = []
    
    # ingest top k indexes from attribution/attention
    for tscript,idx in index_list:
...
def run_attributions(saved_file,df,parent_dir,groups,metrics,mode="attn",layer_idx=0,head_idx=0):
    ...
    positive_indices, negative_indices = top_indices(saved_file,groups,metrics,mode=mode,head_idx=head_idx)
    indices_to_substrings(positive_indices,positive_motifs_file,df)
    indices_to_substrings(negative_indices,negative_motifs_file,df)
    get_positional_bias(positive_indices,negative_indices,df,hist_file,groups,metrics)
...
def get_positional_bias(positive_indices,negative_indices,df_data,hist_file,groups,metrics):
...
def attribution_loci_pipeline(): 
    ...
    #cross_metrics = [['max','max'],['max','min'],['min','max'],['min','min'],['rolling-abs','rolling-abs'],['random','random']]
    cross_metrics = [['rolling-abs','rolling-abs'],['random','random']]
        ...
        for m in metrics:
            ...
            if a != b:
                ...
                for l,f in enumerate(best_BIO_EDA['path_list']):
                    for h in range(8):
                        run_attributions(f,df_test,best_BIO_dir,g,m,mode='attn',layer_idx=l,head_idx=h)
                for l,f in enumerate(best_EDC_EDA['path_list']):
                    for h in range(8):
                        run_attributions(f,df_test,best_EDC_dir,g,m,mode='attn',layer_idx=l,head_idx=h)
