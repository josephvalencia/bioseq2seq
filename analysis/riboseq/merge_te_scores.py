...
#df_score['beam_score'] = [np.exp(x) for x in df_score['beam_score'].values.tolist()]
...
sns.displot(data=limited,x="te",hue="transcript_type")
plt.savefig("te_distribution.pdf")
plt.close()
...
df_all.to_csv("te_coding_scores.txt",sep="\t")

sns.pairplot(data=df_all,hue="transcript_type",vars=["te","coding_prob","beam_score"])
plt.savefig("te_coding_correlations.eps")
