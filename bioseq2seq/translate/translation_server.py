...
from bioseq2seq.constants import DefaultTokens
from bioseq2seq.utils.logging import init_logger
from bioseq2seq.utils.misc import set_random_seed
from bioseq2seq.utils.misc import check_model_config
from bioseq2seq.utils.alignment import to_word_align
from bioseq2seq.utils.parse import ArgumentParser
from bioseq2seq.translate.translator import build_translator
from bioseq2seq.transforms.features import InferFeatsTransform
...
class CTranslate2Translator(object):
    ...
    reproduce the bioseq2seq.translate.translator API.
    """
    ...
    @staticmethod
    def convert_bioseq2seq_to_ct2_opts(ct2_translator_args,
        ...
        bioseq2seq_for_translator = {
            "device": "cuda" if opt.cuda else "cpu",
            "device_index": opt.gpu if opt.cuda else 0,
        }
        for name, value in bioseq2seq_for_translator.items():
            setdefault_if_exists_must_match(
                ct2_translator_args, name, value)

        bioseq2seq_for_translate_batch_enforce = {
        ...
        for name, value in bioseq2seq_for_translate_batch_enforce.items():
            setdefault_if_exists_must_match(
...
class ServerModel(object):
    ...
    def parse_opt(self, opt):
        """Parse the option set passed by the user using `bioseq2seq.opts`
       ...
       Returns:
        ...
        bioseq2seq.opts.translate_opts(parser)
    ...
    def load(self, preload=False):
        ...
        try:
            if self.ct2_model is not None:
                CTranslate2Translator.convert_bioseq2seq_to_ct2_opts(
    ...
    def build_tokenizer(self, tokenizer_opt):
        ...
        if tokenizer_opt['type'] == 'sentencepiece':
        ...
        elif tokenizer_opt['type'] == 'pybioseq2seqtok':
            ...
            tokenizer = pybioseq2seqtok.Tokenizer(mode,
                                            **tokenizer_params)
        else:
    ...
    def tokenize(self, sequence, side='src'):
        ...
        if self.tokenizers_opt[side]["type"] == "sentencepiece":
            ...
            tok = " ".join(tok)
        elif self.tokenizers_opt[side]["type"] == "pybioseq2seqtok":
    ...
    def tokenizer_marker(self, side='src'):
        ...
        if self.tokenizers_opt is not None:
            ...
            if tokenizer_type == "pybioseq2seqtok":
