...
mv tmp*e10.pt bioseq2seq/tests/test_model.pt
rm tmp*.pt
fi
#
# 
...
mv /tmp/tmp*e10.pt bioseq2seq/tests/test_model.pt
...
mv tmp*e8.pt bioseq2seq/tests/test_model2.pt
...
#
if false; then
$my_python translate.py -gpu 0 -model bioseq2seq/tests/test_model.pt \
  -src data/src-val.txt -output bioseq2seq/tests/output_hyp.txt -beam 5 -batch_size 16
