...
def parse_args():
    ...
    parser.add_argument("--num_gpus","--g",type = int,default = 4, help = "Number of GPUs for training")
...
def train_helper(rank,args,seq2seq,generator,random_state):
    ...
    dataframe = filter_by_length(dataframe,1000)
    ...
    train_partitions = partition(train,split_ratios = splits,random_state = random_state) # unique data to each machine
    train_iterator = iterator_from_dataset(train_partitions[rank],8000,device)
...
def train(args):
    ...
    spawn(train_helper, nprocs=n_procs, args=(args,seq2seq,generator,state))
