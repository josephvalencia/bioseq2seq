...
from bioseq2seq.inputters.dataset_base import _dynamic_dict
...
@contextmanager
def maybe_fastafile_open(filename):
    ...
    if filename is None:
        ...
        fa = open(filename)
        _file = SeqIO.parse(fa,"fasta")
    yield _file
    
    if filename is not None and fa:
        fa.close()
...
class ParallelFastaCorpus(ParallelCorpus):
...
class ParallelTextCorpus(ParallelCorpus):
...
class ParallelCorpusIterator(object):
    ...
    def _tokenize(self, stream):
        ...
        for example in stream:
            src_tokenized = example['src'].strip('\n').split()
