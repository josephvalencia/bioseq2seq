...
from bioseq2seq.bin.translate_new import  restore_transformer_model
...
def extract_baselines(batch):
    ...
    stacked = torch.stack(baselines,dim=0).to(device=batch.src[0].device) #,torch.stack(baseline_shapes,dim=0))
    return stacked
    ...
    def run_deeplift(self,savefile,val_iterator,target_pos,baseline):
        ...
        with open(savefile,'w') as outFile:
            for batch in tqdm.tqdm(val_iterator):
                ...
                src = src.transpose(0,1)
                baselines = extract_baselines(batch) 
                ...
                for j in range(batch_size):
                    ...
                    baseline_embed = self.src_embed(baselines[:,:,j,:])
                    print(baseline_embed.shape,curr_src_embed.shape)
    ...
    def run_integrated_gradients(self,savefile,val_iterator,target_pos,baseline):
        ...
        with open(savefile,'w') as outFile:
            for batch in tqdm.tqdm(val_iterator):
                ...
                src = src.transpose(0,1)
                baselines = extract_baselines(batch)
                print(src.shape,baselines[0].shape)
                # can only do one batch at a time
                batch_size = batch.batch_size
                for j in range(batch_size):
                    ...
                    baseline_embed = baselines[0][:,:,j,:]
                    print(j,baseline_embed.shape,curr_src)
                    decoder_input = self.decoder_input(1) 
                    #curr_ids = batch.id
                    curr_tgt = batch.tgt[target_pos,j,:]
                    curr_tgt = torch.unsqueeze(curr_tgt,0)
