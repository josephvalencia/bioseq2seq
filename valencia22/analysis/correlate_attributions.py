...
def calc_correlations(file_a,file_b,mode='spearman',metric='normed_attr'):
    ...
    for la,lb in zip(fh_a.readlines(),fh_b.readlines()):
        ...
        a_good = array_a != 0.0
        b_good = array_b != 0.0
        all_equal =  np.any(np.equal(a_good,b_good))
        print(f'before {fields_a["ID"]}, {len(src_a)}, {len(src_b)}, {array_a.shape}, {array_b.shape}')
        
        if all_equal:
            old_shape = array_a.shape
            array_a = array_a[a_good]
            array_b = array_b[b_good]
            if array_a.shape != array_b.shape:
                print(f'Failed at {fields_a["ID"]}, {len(src_a)}, {len(src_b)}, {array_a.shape}, {array_b.shape}')
            if mode == 'spearman':
...
def compare(regex1,regex2,corr_mode,metric):
...
if __name__ == "__main__":
    ...
    compare(regex1,regex2,'spearman','summed_attr_PC')
